...
public class GroupCourseController {
    ...
    @GetMapping("/all")
    public Result<java.util.List<GroupCourse>> all() {
        return Result.success(groupCourseService.list());
    }

    @GetMapping("/{id}")
    public Result<GroupCourse> detail(@PathVariable Long id) {
        return Result.success(groupCourseService.getById(id));
    }
